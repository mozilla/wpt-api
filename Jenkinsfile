    ...
    stage('Run WebPageTest (command-line API)') {
      ...
      steps {
        writeFile([
          file: 'commands.txt',
          encoding: 'UTF-8',
          text: """test ${TARGET_URL} --location us-east-1-linux:Firefox --bodies --keepua --noimages -r 3 --first --priority 1 --poll 5 --reporter json --label ${TARGET_NAME}.fx.release
test ${TARGET_URL} --location us-east-1-linux:Firefox%20Nightly --bodies --keepua --noimages -r 3 --first --priority 1 --poll 5 --reporter json --label ${TARGET_NAME}.fx.nightly
test ${TARGET_URL} --location us-east-1-linux:Chrome --bodies --keepua --noimages -r 3 --first --priority 1 --poll 5 --reporter json --label ${TARGET_NAME}.chrome.release
test ${TARGET_URL} --location us-east-1-linux:Chrome%20Canary --bodies --keepua --noimages -r 3 --first --priority 1 --poll 5 --reporter json --label ${TARGET_NAME}.chrome.canary"""])
